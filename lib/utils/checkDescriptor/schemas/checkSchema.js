/*
	eslint max-params: [ "error", 7 ], max-statements: 0, max-depth: 0
*/
...
// private
	...
	// methods
		...
		*/
		function _checkSchema (path, method, parameterType, name, _schema, components = {}, forbidenTypes = []) {
			...
			let err = checkNonEmptyObject(fullpath, schema, false);
				...
				if (!err) {
					err = checkType(fullpath, schema.type, forbidenTypes, false);
				}
						...
						if (!err) {
							...
							for (let i = 0; i < propertiesKeys.length; ++i) {
								...
								err = _checkSchema(
									...
									schema.properties[propertiesKeys[i]], components, forbidenTypes
								);
				...
				if (!err && "array" === schema.type) {
					...
					err = _checkSchema(
						path, method, parameterType, name + ".items",
						schema.items, components, forbidenTypes
					);
