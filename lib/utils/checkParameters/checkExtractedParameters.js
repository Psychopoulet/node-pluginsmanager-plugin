		...
		for (let i = 0, paramsKeys = Object.keys(params); i < paramsKeys.length; ++i) {
			...
			switch (docParam.type) {
				...
				case "integer":
					...
					if (!err && docParam.min && result[paramsKeys[i]] < docParam.min) {
					...
					if (!err && docParam.max && result[paramsKeys[i]] > docParam.max) {

						err = new RangeError(
							"\"" + paramsKeys[i] + "\" " + type + " parameter must be" +
							" lower or equals to " + docParam.min
						);
				...
				case "string":
					...
					if (!err && docParam.min && result[paramsKeys[i]].length < docParam.min) {
					...
					if (!err && docParam.max && result[paramsKeys[i]].length > docParam.max) {
