...
class Server extends MediatorUser_1.default {
    ...
    _getSocketIOVersion() {
        if (this._socketServer) {
            if ("function" === typeof this._socketServer.sockets?.sockets?.has) {
                return "V3-V4";
            }
            else {
                return "V2";
    ...
    _getUsableSocketIOClient(clientId) {
        if ("SOCKETIO" === this._getServerType()) {
            ...
            if ("V2" === socketIOVersion) {
                const sockets = this._socketServer.sockets.sockets;
    ...
    appMiddleware(_req, res, next) {
        ...
        this.checkDescriptor().then(() => {
            ...
            if (!this._Descriptor.paths?.[req.pattern]?.[req.method]) {
                return next();
            }
            const operation = this._Descriptor.paths[req.pattern][req.method];
