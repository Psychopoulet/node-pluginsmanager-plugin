...
// deps
	...
	const { extractPathName, extractParams, extractBody } = require(join(__dirname, "..", "utils", "descriptor", "main.js"));
	...
	// public
		...
		appMiddleware (req, res, next) { // req, res, next : void
			...
			this.checkDescriptor().then(() => {
				...
				req.headers = req.headers ? req.headers : {}; // @TODO ??
				req.cookies = req.cookies ? req.cookies : {}; // @TODO
				req.pathname = req.pathname ? req.pathname : pathname;
				req.query = req.query ? req.query : query || {};
				req.method = req.method ? req.method : req.method.toLowerCase();
				req.ip = req.ip ? req.ip : ((req.headers["x-forwarded-for"] || "").split(",").pop() ||
				...
				req.path = req.path ? req.path : extractPathName(this._Descriptor.paths, req.pathname, req.method);

				if (!req.path || !this._Descriptor.paths[req.path] || !this._Descriptor.paths[req.path][req.method]) {
					return next();
				}

				const { operationId } = this._Descriptor.paths[req.path][req.method];

				this._log("info", "" +
					"=> [" + req.ip + "] " + req.url + " (" + req.method.toUpperCase() + ")" +
				...
				if ("/" + this._Descriptor.info.title + "/descriptor" === req.path && "get" === req.method) {

					this._log("info", "<= [" + req.ip + "] " + JSON.stringify(this._Descriptor));
				...
				// missing operationId
					...
					this._log("error", "<= [" + req.ip + "] " + JSON.stringify(result));
				...
				// "Content-Length" header is not a number
					...
					req.params = req.params ? req.params : extractParams(req.path, req.pathname);

					Promise.resolve().then(() => {

						return req.body ? Promise.resolve() : extractBody(req).then((body) => {
						...
						return Promise.resolve().then(() => {
							...
							return this._checkParameters ? this._Mediator.checkParameters(
								operationId, req.query, req.body, contentType
							).then((parsedData) => {
								parsed = parsedData;
							}) : Promise.resolve();
