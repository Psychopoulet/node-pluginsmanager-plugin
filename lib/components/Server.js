...
// deps
	...
	const { checkInteger } = require(join(__dirname, "..", "checkers", "main.js"));
	...
	// public
		...
		appMiddleware (req, res, next) { // req, res, next : void
			...
			this.checkDescriptor().then(() => {
				...
				req.cookies = _isNonEmptyObject(req.cookies) ? req.cookies : extractCookies(req);
				...
				// no "Content-Length" header found
					...
					new Promise((resolve) => {
						...
						req.headers["content-length"] = "string" === typeof req.headers["content-length"] ?
							parseInt(req.headers["content-length"], 10) :
							req.headers["content-length"];

						checkInteger("Content-Length", req.headers["content-length"]).then(resolve).catch((err) => {
