		...
		checkParameters (operationId, urlParams, bodyParams, contentType) {
			...
			return this.checkDescriptor().then(() => {
				...
				// compare body params length to doc
					...
					const bodyParamsLength = Object.keys(bodyParams || {}).length;

					if (bodyParamsDoc.length && !bodyParamsLength) {
						return Promise.reject(new ReferenceError("Missing body parameters"));
					}
					else if (!bodyParamsDoc.length && bodyParamsLength) {

						return Promise.reject(new Error(
							"Not wanted body parameters :" +
							"[ \"" + Object.keys(bodyParams || {}).join("\", \"") + "\" ]"
						));
